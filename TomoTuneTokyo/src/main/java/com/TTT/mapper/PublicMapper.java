...
public interface PublicMapper {
    ...
    // 카테고리에 해당하는 글 불러오기
    @Select("SELECT p.post_id, p.post_title, p.board_id, p.user_id, p.post_pinned, p.created_at, u.nickname FROM posts p, user_profile u WHERE board_id = #{board_id} AND p.user_id = u.user_id AND (p.post_status IS NULL OR p.post_status != 'D') ORDER BY p.created_at desc")
    List<PostVo> getPostList(int board_id);

    // 모든 글 불러오기
    @Select("SELECT p.post_id, p.post_title, p.post_like, p.progress, p.board_id, p.user_id, p.post_pinned, p.created_at, u.nickname, (SELECT COUNT(*) FROM comments c, posts p WHERE p.post_id = c.post_id) comments FROM posts p, user_profile u WHERE NOT board_id = 7 AND p.user_id = u.user_id AND (p.post_status IS NULL OR p.post_status != 'D') ORDER BY p.created_at desc")
