...
function renderEmailStep(username) {
    ...
    $("#fpSendCode").on("click", async () => {
        ...
        if (!email) {
            return $("#fpEmailMsg").text("メールアドレスを入力してください").css("color", "red");
        }

        try {
            // ① 아이디–이메일 매칭 체크
            const matchRes = await fetch(`/user/check-username-email?username=${encodeURIComponent(fpUsernameGlobal)}&email=${encodeURIComponent(email)}`);
            const isMatch = await matchRes.json();
            if (!isMatch) {
                return $("#fpEmailMsg").text("IDとメールアドレスが一致しません").css("color", "red");
            }

            await $.post("/user/send-email-code", { email });
            $("#fpEmailMsg").text("認証メールを送信しました").css("color", "white");
            $("#fpCodeGroup").show();
            $("#fpEmail").prop("disabled", true);
        } catch (e) {
            console.error(e);
            $("#fpEmailMsg").text("通信エラーが発生しました").css("color", "red");
        }
    });
    // 코드 검증 클릭
